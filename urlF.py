...
class EnhancedConfig:
    ...
    MAX_WORKERS = min(32, (os.cpu_count() or 1) + 4)  # Better default
    ...
    def validate_workers(cls, workers: int) -> int:
        ...
        return max(1, workers)  # Allow user to specify as many workers as they want
...
class EnhancedURLValidator:
    ...
    @classmethod
    def ensure_scheme(cls, url: str) -> str:
    ...
    def is_valid_url(cls, url: str) -> bool:
        ...
        try:
            ...
            if scheme not in EnhancedConfig.VALID_SCHEMES:
                return False
            
            # Schemes that don't strictly require netloc
            if scheme in {'file', 'data'}:
                return True
                
            # Validate netloc for other schemes
            if not result.netloc:
                return False
            
            # Extract host and port using built-in properties
...
class EnhancedURLProcessor:
    ...
    def process_url_batch(self, urls: List[str]) -> Tuple[Dict, List[Dict], int, int]:
        ...
        for url in urls:
            try:
                # Ensure scheme exists before processing
                url_with_scheme = EnhancedURLValidator.ensure_scheme(url)
                
                # Validate original URL
                if not EnhancedURLValidator.is_valid_url(url_with_scheme):
                    invalid_count += 1
                    continue
                
                # Normalize URL
                normalized = EnhancedURLValidator.normalize_url(url_with_scheme)
                if normalized != url_with_scheme:
                    normalized_count += 1
    ...
    def process_file(self, input_file: str, output_file: str, 
        ...
        # We don't read the whole file first to avoid double disk I/O on huge files.
        logger.info(f"Processing URLs with {self.max_workers} workers...")
        self.stats["total"] = 0
        
        # Process URLs in batches
        processed_urls = []
        
        with tqdm(desc="Processing URLs", unit="URL") as pbar:
            if use_threads:
                # Multithreaded processing
                import concurrent.futures
                with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                    # Submit batches to executor
                    batch_size = EnhancedConfig.CHUNK_SIZE
                    active_futures = set()
                    ...
                    def process_future(future):
                        try:
                            ...
                            if len(self.duplicate_samples) < 5:
                    ...
                    for url in EnhancedFileHandler.read_urls_safely(input_file):
                        ...
                        if len(current_batch) >= batch_size:
                            ...
                            # Prevent memory overload by waiting if queue is full
                            while len(active_futures) >= self.max_workers * 2:
        ...
        self.stats["urls_per_second"] = self.stats["total"] / self.stats["processing_time"] if self.stats["processing_time"] > 0 else 0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A tool to filter and deduplicate URLs based on domain, path, and query parameter NAMES only.",
        epilog="""
USAGE EXAMPLES:
  Basic usage:
...
HOW IT WORKS:
  • URLs are deduplicated based on domain + path + parameter names (not values)
